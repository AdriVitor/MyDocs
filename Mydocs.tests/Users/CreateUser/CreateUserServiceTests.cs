...
namespace MyDocs.Tests.Users.CreateUser
{
    public class CreateUserServiceTests
    {
        ...
        public async Task AddUser_ShouldCreateUserAndCredentialsAndSendEmail_WhenRequestIsValid()
        {
            ...
            emailTemplateServiceMock.Setup(x => x.ReplaceEmailTemplateWelcome(It.IsAny<EmailTemplate>(), request.Name))
                                    .Returns(template);
            ...
            emailServiceMock.Verify(e => e.SendEmail(
                request.Email,
                subject,
                $"<h1> Ola {request.Name}, Parabéns pela sua criação de conta no MyDocs </h1>"), Times.Once);
        }
