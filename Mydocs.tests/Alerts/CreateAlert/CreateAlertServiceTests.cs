...
using MyDocs.Shared.Services.EmailTemplates;
...
namespace MyDocs.Tests.Alerts.CreateAlert
{
    public class CreateAlertServiceTests
    {
        ...
        public async Task AddAlert_ShouldCreateAlertSuccessfully()
        {
            ...
            emailTemplateServiceMock.Setup(x => x.FindEmailTemplate(EmailTemplateEnum.Overdue_Bill))
            ...
            emailTemplateServiceMock.Setup(x => x.ReplaceEmailTemplateOverdueBill(It.IsAny<EmailTemplate>(), userName, request.Name))
            ...
            emailTemplateServiceMock.Verify(x => x.FindEmailTemplate(EmailTemplateEnum.Overdue_Bill), Times.Once);
            emailTemplateServiceMock.Verify(x => x.ReplaceEmailTemplateOverdueBill(It.IsAny<EmailTemplate>(), userName, request.Name), Times.Once);
        }
    }
}
